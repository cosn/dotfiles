---
name: prr
description: Use when the user asks for a deep GitHub PR or branch code review. Resolve the target, inspect changed files in context, run correctness/design/test/style/performance review passes, triage existing PR comments, and optionally post GitHub review comments after user approval.
---

# Review

Use the PR number, PR URL, branch name, or current branch implied by the user's request.

## Resolving the Target

1. If the user provided a PR number or URL, fetch the diff and metadata. Run these commands:
...
2. If the user provided a branch name, diff against the base branch. Run:
   - git diff main...<branch_name>

3. If the user did not provide a target, diff the current branch against main. Run:
   - git diff main...HEAD
...
How to read changed files depends on which mode resolved above:
...
Keep the mode, refs, diff, and changed file list visible while reviewing so every pass reads the correct source. For grepping the broader codebase (e.g., finding existing patterns), always use the base ref as the reference point.

## Analysis Passes

Run every pass below against the changed files identified above. If the user explicitly asked for subagents or parallel agent work, split independent passes across subagents. Otherwise, perform the passes locally and use shell-level parallelism for independent file reads/searches when useful.
