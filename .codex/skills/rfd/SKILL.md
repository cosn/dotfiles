...
Ask one focused question at a time; never batch. If local code, linked docs, tickets, or previous context can answer the question, inspect those first instead of asking.
...
Use the user's request as the problem statement. Interview in detail about technical implementation, UI and UX, concerns, tradeoffs, and anything else needed. Make sure questions are not obvious. Go in-depth and continue interviewing until you can write the RFD confidently. Then write the RFD to `/tmp/yyyy-mm-dd-<slug>.md`.
